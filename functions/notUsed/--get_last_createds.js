...
const getLastCreateds = (async (req, res, next,model) => {
    await connect();
    ...
    data = await model.model.find(query).select(select).sort({"createdAt":-1}).limit(limit).skip(skip)
        .then(async data => {
            response = {
                code: 200 ,
                status: 'success',
...
module.exports = getLastCreateds;